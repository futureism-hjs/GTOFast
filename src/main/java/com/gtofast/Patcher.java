...
/**
 * Batch recipe patcher — handles GT recipe data and vanilla recipes.
 */
final class Patcher {
    ...
    static void patchGTRecipes(MinecraftServer server) {
        ...
        LOGGER.info("[GTOFast] Collected {} GT recipe objects", recipes.size());
        if (recipes.isEmpty()) { LOGGER.warn("[GTOFast] No GT recipes found"); return; }
        ...
        for (int i = 0; i < recipes.size(); i++) {
            ...
            try {
                ...
                if ((i + 1) % 5000 == 0) {
                    LOGGER.info("[GTOFast] progress {}/{} d={}", i + 1, recipes.size(), patched);
                }
            } catch (Exception e) {
                if (++errors <= 5) LOGGER.warn("[GTOFast] GT patch err: {}", e.getMessage());
            }
        }
        LOGGER.info("[GTOFast] GT patch: {}/{} patched {} gen {} errs {}ms",
    ...
    private static List<Object> collectAllGTRecipes(MinecraftServer server) {
        ...
        try {
            ...
            LOGGER.info("[GTOFast] RECIPE_TYPES count: {}", types.size());
            ...
            for (Object type : types) {
                ...
                if (isGeneratorTypeName(typeName)) {
                    LOGGER.info("[GTOFast] Skipping generator type: {}", typeName);
                    continue;
                }
            ...
            LOGGER.info("[GTOFast] recipe types: {} hit, total recipes: {}", typeHits, all.size());
        } catch (Exception e) {
            LOGGER.warn("[GTOFast] GTRegistries enumeration failed: {}", e.getMessage());
        }
        ...
        if (all.isEmpty()) {
            ...
            LOGGER.info("[GTOFast] RecipeManager fallback: {} recipes", all.size());
        }
    ...
    private static Collection<Object> collectViaGTORecipeTypes() {
        ...
        try {
            ...
            LOGGER.info("[GTOFast] GTORecipeTypes fallback: {} recipes", all.size());
        } catch (Exception e) {
            LOGGER.warn("[GTOFast] GTORecipeTypes fallback failed: {}", e.getMessage());
        }
        return all;
    }
    ...
    static void patchVanillaRecipes(MinecraftServer server) {
        ...
        for (Recipe<?> r : server.getRecipeManager().getRecipes()) {
            ...
            try {
                ...
                if (++errs <= 5) LOGGER.warn("[GTOFast] Vanilla err: {}", e.getMessage());
            }
        }
        LOGGER.info("[GTOFast] Vanilla patch: {} patched {} errs", count, errs);
    }
